...
class AcpClient {
  ...
  _handleAgentRequest(msg) {
    ...
    // Unknown agentâ†’client request: empty result so agent does not hang
...
function slimToolArgs(args) {
  ...
  for (const k of keys) {
    ...
    let v = args[k];
    if (typeof v === 'string' && v.length > 240) v = `${v.slice(0, 240)}â€¦`;
    out[k] = v;
  }
  if (!Object.keys(out).length) {
    try {
      const s = JSON.stringify(args);
      return { preview: s.length > 300 ? `${s.slice(0, 300)}â€¦` : s };
    } catch {
      return {};
    }
  }
